...
JSON_FILE = os.path.join(backend_dir, "data", "s24_clean_with_grades.json")
...
def generate_response(query, context):
    ...
    system_message = """You are an AI assistant for a university information system. Your role is to provide helpful information based on the given context and the user's original query. Follow these guidelines:
    1. Answer the user's question using the information provided in the courses context, but always keep the original query in mind.
    2. If the query is about general assistance or resources, provide general information without mentioning specific courses unless explicitly asked.
    3. Use a friendly, professional tone appropriate for a university setting.
    4. Only mention specific courses if they are directly relevant to the user's query or if the user specifically asks for course recommendations.
    5. If the courses context doesn't contain enough information to answer the question, say so and suggest where the student might find more information (e.g., academic advisors, department websites).
    6. Keep your response concise and to the point, typically within 2-4 sentences unless more detail is necessary.
    7. If appropriate, suggest general resources or departments that might be helpful, rather than specific courses.
    8. Always ensure your response directly addresses the user's original question.

    Remember, your goal is to help students find the information they need, which may not always involve recommending specific courses."""

    user_message = f"""Courses Context:
{context}

Original User Question: {query}

    Please provide a response based on the above courses context and the original user question, following the guidelines in your instructions."""
...
def rag_pipeline(query, indexer):
    ...
    # Preprocess query for search purposes only
    preprocessed_query = preprocess_query(query)
    
    # Perform searches using the preprocessed query
